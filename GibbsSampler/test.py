...
def main(arg='.'):
    ...
    query(queries='Cancer,Smokes,Friends', method='GibbsSampler', mln=mln, db=db, verbose=False, multicore=True).run()
    t1 = time.time()-start
    ...
    query(queries='Cancer,Smokes,Friends', method='GibbsSampler', mln=mln, db=db, verbose=False, multicore=False).run()
    t2 = time.time()-start
    #print('multi core exact inference test: {}'.format(t))
    print('Inference, GibbsSampler, {}, {}'.format(t1, t2))
        #fl.write(str(t))
        #fl.write('\t(MultiCore)\n')
